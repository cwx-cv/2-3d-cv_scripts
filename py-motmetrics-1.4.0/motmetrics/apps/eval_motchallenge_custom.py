...
string.""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('groundtruths', type=str, help='Directory containing ground truth files.') ### gt.txt文件
    parser.add_argument('tests', type=str, help='Directory containing tracker result files') ### track.txt文件
...
def compare_dataframes(gts, ts, iouth, distth): ### 修改了这个函数
    ...
    for k, tsacc in ts.items():
        if k in gts:
            if iouth > 0:
                logging.info('Comparing IoU distance...', k)
                accs.append(mm.utils.compare_to_groundtruth(gts[k], tsacc, 'iou', distth=1-iouth))
            else:
                logging.info('Comparing Euclidean distance...', k)
...
def main():
    ...
    # 设置min_confidence=-1 在进行评估时, 含义：只考虑置信度在min_confidence以上的哪些跟踪框
